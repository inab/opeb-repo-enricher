...
class RepoEnricher:
    ...
    MATCHERS: "Sequence[Tuple[str, str]]" = [
        ("opeb_repo_enricher.repo_matcher.github", "GitHubRepoMatcher"),
        ("opeb_repo_enricher.repo_matcher.bitbucket", "BitBucketRepoMatcher"),
    ]

    def __init__(
        self,
        config: "configparser.ConfigParser",
        p_matchers: "Sequence[Tuple[str, str]]" = MATCHERS,
    ):
        ...
        rmInstances: "MutableSequence[AbstractRepoMatcher]" = list()
        for rmmodule_name, rmclazz_name in p_matchers:
            ...
            try:
                ...
                if not issubclass(rmclazz, AbstractRepoMatcher):
                    raise ValueError(
                        f"{rmclazz.__class__.__name__} is not a subclass of AbstractRepoMatcher"
                    )

                rmInstances.append(rmclazz(config))
            except Exception as e:
                raise RepoEnricherException(
                    f"Unable to load module {rmmodule_name} or class {rmclazz_name} ({str(e)} see stack trace)"
                ) from e

        self.repo_matchers = rmInstances

    def analyzeOpenEBenchEntries(
        self, oebQ: "OpenEBenchQueries"
    ) -> "Iterator[Tuple[str, Sequence[str], Sequence[Mapping[str, Any]]]]":
    ...
    def analyzeRepositoriesList(
        self, repo_links: "Iterable[str]"
    ) -> "Iterator[Tuple[str, Sequence[str], Sequence[Mapping[str, Any]]]]":
        for repo_link in repo_links:
            entry_links = [repo_link]
            yield repo_link, entry_links, self.parsePutativeURLs(repo_link, entry_links)

    def parsePutativeURLs(
        self, entry_id: "str", entry_links: "Sequence[str]"
    ) -> "Sequence[Mapping[str, Any]]":
        queries: "MutableSequence[Mapping[str, Any]]" = list()

        if len(entry_links) > 0:
            repoEntries: "MutableMapping[str, MutableMapping[str, Any]]" = dict()
            repos = list()
            ...
            for entry_link in entry_links:
                for rm in self.repo_matchers:
                    ...
                    if (
                        ...
                        p_links = (
                            repoEntries.setdefault(kind, dict())
                            .setdefault(lcWorkspace, dict())
                            .get(lcRepo)
                        )
                        if p_links is None:
                            p_links = list()
            ...
            if len(repos) == 0:
                self.logger.info(
                    f"No identified repo for {entry_id}: (links {entry_links})"
                )
            queries.append(
                {"@id": entry_id, "entry_links": entry_links, "repos": repos}
            )
