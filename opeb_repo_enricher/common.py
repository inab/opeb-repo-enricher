...
def full_http_read(req: "response.addinfourl") -> "bytes":
    # The original bytes
    response = b""
    while True:
        try:
...
def _parse_isoformat_time(
    tstr: "str",
) -> "Tuple[int, int, int, int, Optional[datetime.tzinfo]]":
    ...
    time_comps: "MutableSequence[Union[int, Optional[datetime.tzinfo]]]" = list(
    ...
    return cast(
        "Tuple[int, int, int, int, Optional[datetime.tzinfo]]", tuple(time_comps)
    )
...
def _parse_hh_mm_ss_ff(tstr: "str") -> "Tuple[int,int,int,int]":
    ...
    return cast("Tuple[int,int,int,int]", tuple(time_comps))


def jsonFilterEncode(
    colKwArgs: "Any", sort_keys: "bool" = False, indent: "Optional[int]" = None
) -> "str":
    """ """
    return json.dumps(
        colKwArgs, cls=ResultJSONEncoder, sort_keys=sort_keys, indent=indent
    )


def jsonFilterStreamEncode(
    colKwArgs: "Any",
    ...
    """ """
    json.dump(
        colKwArgs, fp=fp, cls=ResultJSONEncoder, sort_keys=sort_keys, indent=indent
    )


def load_with_datetime(
    pairs: "Sequence[Tuple[str, Any]]", tz: "Optional[datetime.timezone]" = None
) -> "Mapping[str, Any]":
    """Load with dates"""
...
def jsonFilterDecode(
    strColKwArgs: "str", tz: "Optional[datetime.timezone]" = None
) -> "Any":
    """ """
    return json.loads(
        strColKwArgs, object_pairs_hook=lambda x: load_with_datetime(x, tz)
    )
