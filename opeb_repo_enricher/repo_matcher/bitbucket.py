...
from .abstract import AbstractRepoMatcher
...
class BitBucketRepoMatcher(AbstractRepoMatcher):
    ...
    def __init__(self, config: "configparser.ConfigParser"):
        ...
        self.bitbucketRepoDataCache: "MutableMapping[str, MutableMapping[str, Any]]" = (
            dict()
        )
        self.bitbucketUserCache: "MutableMapping[str, Mapping[str, Any]]" = dict()

        # Needed to set up the size of the requested batches of data
        self.pageSize: "int" = config.getint(
            self.BITBUCKET_KIND, "page-size", fallback=100
        )
    ...
    def _getCredentials(self) -> "Tuple[str, Optional[str], Optional[str]]":
        ...
        return self.BITBUCKET_API_ENDPOINT, bb_user, bb_token
                        ...
                        if repoSlug is None:
                            ...
                            try:
                                with urllib.request.urlopen(req) as response:
                                    ...
                                    matched = self.BITBUCKET_COMPILED_PATTERN.match(
                                        page
                                    )
    ...
    def getBitBucketUser(
        self, username: "str", endpoint: "Optional[str]" = None
        ...
        if userData is not None:
            ...
            userUri = urllib.parse.urljoin(
                self.BITBUCKET_API_ENDPOINT,
                "/".join(
                    map(lambda p: urllib.parse.quote(p, safe=""), ["users", username])
                ),
            )

            userDataList: "Union[Sequence[Mapping[str, Any]], Mapping[str, Any]]"
            userSuccess, userDataList = self.fetchJSON(
    ...
    def getRepoData(self, fullrepo: "Mapping[str, Any]") -> "Mapping[str, Any]":
        workspace: "str" = fullrepo["owner"]
        repoSlug: "str" = fullrepo["repo"]
        ...
        ans: "Optional[MutableMapping[str, Any]]" = p_bitbucketRepoDataCache.setdefault(
            lcWorkspace, dict()
        ).get(lcRepoSlug)
        ...
        if ans is None:
            ...
            if repoSuccess:
                ...
                realOwner: "Optional[str]" = None
                lcRealOwner: "Optional[str]" = None
                realWorkspace: "Optional[str]" = None
                realRepoSlug: "Optional[str]" = None
                lcRealRepoSlug: "Optional[str]" = None
                ...
                if isinstance(ownerH, dict):
                    ...
                    if userKey is not None:
                        ...
                        if realWorkspace is not None:
                            p_bitbucketRepoDataCache[realWorkspace] = (
                                p_bitbucketRepoDataCache[lcRealOwner]
                            )
                    else:
                        import json

                        self.logger.error(
                            f"ASSERT: Tell the developer about this entry ({workspace} {repoSlug})\n\n{json.dumps(ownerH, sort_keys=True, indent=4)}"
                        )

                if realRepoSlug is not None:
                    assert lcRealRepoSlug is not None
                    assert lcRealOwner is not None
                ...
                if followProcessing and isinstance(linksH, dict):
                    ...
                    if isinstance(cloneA, list):
                        for cloneP in cloneA:
                            ...
                            if (cloneP_href is not None) and cloneP.get(
                                "name"
                            ) == "https":
                                ...
                                if cP_usermark != -1:
                                    ...
                                    cloneP_href = urllib.parse.urlunparse(
                                        (cP[0], cP_netloc, cP[2], cP[3], cP[4], cP[5])
                                    )
                    ...
                    ans["source_hasAnonymousAccess"] = not repoData.get(
                        "is_private", True
                    )
                    ...
                    if repoData.get("has_issues", False):
                        ...
                        if isinstance(issuesH, dict):
                            ...
                            assert realOwner is not None
                            assert realRepoSlug is not None
                            issuesUri = urllib.parse.urljoin(
                                self.BITBUCKET_API_ENDPOINT,
                                "/".join(
                                    map(
                                        lambda p: urllib.parse.quote(p, safe=""),
                        ...
                        while issuesUri is not None:
                            ...
                            if issuesSuccess:
                                ...
                                for issue in issuesData.get("values", list()):
                                    ...
                                    if (issueCreated is not None) and (
                                        (lastCreated is None)
                                        or lastCreated < issueCreated
                                    ):
                                        lastCreated = issueCreated

                                    issueUpdated = issue.get("updated_on")
                                    if (issueUpdated is not None) and (
                                        (lastUpdated is None)
                                        or lastUpdated < issueUpdated
                                    ):
                                        lastUpdated = issueUpdated
                                    if issue.get("state") in ("closed", "resolved"):
                                        numClosed += 1
                                        closedDt = datetimeFromISOFormat(
                                            issue["updated_on"]
                                        )
                                        createdDt = datetimeFromISOFormat(
                                            issue["created_on"]
                                        )
                                        ...
                                        if (
                                            not isinstance(
                                                minDuration, datetime.timedelta
                                            )
                                            or duration < minDuration
                                        ):
                                            minDuration = duration

                                        if (
                                            not isinstance(
                                                maxDuration, datetime.timedelta
                                            )
                                            or maxDuration > duration
                                        ):
                        ...
                        if minDuration is not None:
                            ...
                            issues["meanDuration"] = datetime.timedelta(
                                seconds=meanSeconds
                            )
                            issues["medianDuration"] = datetime.timedelta(
                    ...
                    if releasesUri is not None:
                        ...
                        releasesUri += "?" + urllib.parse.urlencode(
                            {self.BITBUCKET_PAGELEN: self.pageSize}
                        )
                    ...
                    if tagsUri is not None:
                        ...
                        tagsUri += "?" + urllib.parse.urlencode(
                            {self.BITBUCKET_PAGELEN: self.pageSize}
                        )
                    ...
                    if isinstance(language, str) and len(language) > 0:
                        ...
                        for lang in ans["languages"]:
                            ...
                            interpreted = interpreted or (
                                lcLang in self.recognizedInterpretedLanguages
                            )
                            compiled = compiled or (
                                lcLang in self.recognizedCompiledLanguages
                            )
                        ...
                        tool_buildSystem = self.recognizedBuildSystemsByLang.get(
                            lcLanguage
                        )
                    ...
                    if contrUri is not None:
                        ...
                        contrUri += "?" + urllib.parse.urlencode(
                            {self.BITBUCKET_PAGELEN: self.pageSize}
                        )
                        ...
                        while contrUri is not None:
                            ...
                            if contrSuccess:
                                ...
                                for commit in contrData.get("values", list()):
                                    ...
                                    ematch = self.EMAIL_COMMIT_COMPILED_PATTERN.match(
                                        name
                                    )
                    ...
                    if watchersUri is not None:
                        ...
                        watchersSuccess, watchersDataF = self.fetchJSON(
                            watchersUri,
                            p_acceptHeaders=self.BITBUCKET_API_ACCEPT_HEADER,
                        )
                    ...
                    if branchesUri is not None:
                        ...
                        branchesUri += "?" + urllib.parse.urlencode(
                            {self.BITBUCKET_PAGELEN: self.pageSize}
                        )
                        ...
                        while branchesUri is not None:
                            branchesSuccess, branchesDataF = self.fetchJSON(
                                branchesUri,
                                p_acceptHeaders=self.BITBUCKET_API_ACCEPT_HEADER,
                            )
                            ...
                            if branchesSuccess:
                                ...
                                for branch in branchesData.get("values", list()):
                                    p_br = {"name": branch["name"]}
                                    branches.append(p_br)

                                    target_date = branch.get("target", dict()).get(
                                        "date"
                                    )
            ...
            if minProcessing:
                ...
                # Alternate way
                clone_uri = urllib.parse.urljoin(
