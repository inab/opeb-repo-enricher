...
class GitHubRepoMatcher(AbstractRepoMatcher):
    ...
    GITHUB_PATTERN = r":\/\/" + GITHUB_HOST + r'\/([^"\'\/]+)\/([^"\'\/]+)'
    ...
    def __init__(self, config: configparser.ConfigParser):
        ...
        self.githubRepoDataCache: "MutableMapping[str, MutableMapping[str, Any]]" = (
            dict()
        )
        self.githubUserCache: "MutableMapping[str, Mapping[str, Any]]" = dict()
    ...
    def _getCredentials(self) -> "Tuple[str, Optional[str], Optional[str]]":
        ...
        return self.GITHUB_API_ENDPOINT, gh_user, gh_token

    def _quotaData(self) -> "Mapping[str, Any]":
        quotaSuccess, quotaData = self.fetchJSON(
            self.GITHUB_API_RATE_LIMIT,
            p_acceptHeaders=self.GITHUB_API_V_HEADER,
    ...
    def getNumReqAndReset(self) -> "Tuple[int, int]":
        ...
        if self._remaining is None:
            ...
            assert self._resettime is not None
        assert self._resettime is not None
    ...
    def getGitHubUser(
        ...
        if userData is None:
            ...
            userSuccess, userDataL = self.fetchJSON(
    ...
    def getRepoData(self, fullrepo: "Mapping[str, Any]") -> "Mapping[str, Any]":
        ...
        ans: "Optional[MutableMapping[str, Any]]" = p_githubRepoDataCache.setdefault(
            lcOwner, dict()
        ).get(lcRepo)
        ...
        if ans is None:
            ...
            if repoSuccess:
                ...
                if followProcessing:
                    ...
                    ans["has_help"] = repoData.get("has_wiki", False) or repoData.get(
                        "has_pages", False
                    )
                    # canonical|version:Tutorial
                    ans["has_tutorial"] = repoData.get(
                        "has_wiki", False
                    ) or repoData.get("has_pages", False)
                    ...
                    if repoData.get("has_issues", False):
                        ...
                        issuesUri = urllib.parse.urljoin(
                            ...
                            "/".join(
                                map(
                                    lambda p: urllib.parse.quote(p, safe=""),
                                    ["", "repos", realOwner, realRepo, "issues"],
                                )
                            )
                            + "?"
                        ...
                        if issuesSuccess:
                            ...
                            for issue in issuesData:
                                if issue["state"] == "closed":
                                    ...
                                    if (
                                        not isinstance(minDuration, datetime.timedelta)
                                        or duration < minDuration
                                    ):
                                        minDuration = duration

                                    if (
                                        not isinstance(maxDuration, datetime.timedelta)
                                ...
                                if (issueCreated is not None) and (
                                    (lastCreated is None) or lastCreated < issueCreated
                                ):
                                    lastCreated = issueCreated

                                issueUpdated = issue.get("updated_at")
                                if (issueUpdated is not None) and (
                                    (lastUpdated is None) or lastUpdated < issueUpdated
                                ):
                                    lastUpdated = issueUpdated

                                issueClosed = issue.get("closed_at")
                                if (issueClosed is not None) and (
                                    (lastClosed is None) or lastClosed < issueClosed
                                ):
                    ...
                    releasesUri = urllib.parse.urljoin(
                        self.GITHUB_API_ENDPOINT,
                        "/".join(
                            map(
                                lambda p: urllib.parse.quote(p, safe=""),
                                ["", "repos", realOwner, realRepo, "releases"],
                            )
                        ),
                    )
                    ...
                    tagsUri = urllib.parse.urljoin(
                        self.GITHUB_API_ENDPOINT,
                        "/".join(
                            map(
                                lambda p: urllib.parse.quote(p, safe=""),
                                ["", "repos", realOwner, realRepo, "tags"],
                            )
                        ),
                    )
                    ...
                    langsUri = urllib.parse.urljoin(
                        self.GITHUB_API_ENDPOINT,
                        "/".join(
                            map(
                                lambda p: urllib.parse.quote(p, safe=""),
                                ["", "repos", realOwner, realRepo, "languages"],
                            )
                        ),
                    )
                    langsSuccess, langsDataF = self.fetchJSON(
                    ...
                    if langsSuccess:
                        ...
                        for (
                            lang,
                            build_tool,
                        ) in self.recognizedBuildSystemsByLang.items():
                    ...
                    contrUri = urllib.parse.urljoin(
                        self.GITHUB_API_ENDPOINT,
                        "/".join(
                            map(
                                lambda p: urllib.parse.quote(p, safe=""),
                                ["", "repos", realOwner, realRepo, "contributors"],
                            )
                        ),
                    )
                    ...
                    if ans["has_license"]:
                        licUri = urllib.parse.urljoin(
                            self.GITHUB_API_ENDPOINT,
                            "/".join(
                                map(
                                    lambda p: urllib.parse.quote(p, safe=""),
                                    ["", "repos", realOwner, realRepo, "license"],
                                )
                            ),
                        )
                    ...
                    commitUri = urllib.parse.urljoin(
                        ...
                        "/".join(
                            map(
                                lambda p: urllib.parse.quote(p, safe=""),
                                ["", "repos", realOwner, realRepo, "commits"],
                            )
                        ),
                    )
                    ...
                    if commitSuccess and len(commitData) > 0:
                        ...
                        treeSuccess, treeDataF = self.fetchJSON(
                            treeUri, p_acceptHeaders=self.GITHUB_API_V_HEADER
                        )
                        if isinstance(treeDataF, list):
                            treeData = treeDataF[0]
                        else:
                            treeData = cast("Mapping[str, Any]", treeDataF)
                        if treeSuccess:
